...
max_new_tokens = 1500  # Further increased to allow for complete dialogs
temperature = 0.6     # For coherent outputs
top_k = 30            # For less nonsense
...
def generate_prompt(scenario=None):
    ...
    prompt = (
        f"scenario:{scenario}\n"
        "Generate a coherent, realistic, and complete 999 emergency call dialogue for the given scenario.\n"
        "The dialogue must alternate strictly between operator: and caller: lines.\n"
        ...
        "Caller: Provides clear, logical information without nonsense or mixing scenarios, describes situation step-by-step.\n"
        "End the dialogue naturally when help is dispatched, with phrases like 'thank you', 'goodbye', or 'bye' from either party. Do not continue after that. No incomplete sentences at the end.\n"
...
# -------------------- MAIN GENERATION --------------------
prompt, scenario = generate_prompt()  # Or specify e.g., generate_prompt("hazardous_material")
logger.info(f"Generating dialog for scenario: {scenario}")

input_ids = torch.tensor([encode(prompt)], dtype=torch.long).to(device)
generated_ids = model.generate(input_ids, max_new_tokens, temperature=temperature, top_k=top_k, repetition_penalty=repetition_penalty)
...
# Ensure it ends completely, add period if needed but no extra text
