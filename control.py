...
# ------------------------------------------------------------------
def process_audio_file(audio_indices, shared_data, overwrite=False):
    ...
    for audio_index in audio_indices:
        ...
        for seg in segments:
            ...
            if bert_role in ["operator", "caller"]:
                ...
                role = "operator"
            elif emotion in {"anger", "fear", "sadness"} and any(p in txt_lower for p in ["i", "my", "he", "she"]):
                role = "caller"
...
_FINETUNE_CHUNK      = 1           # trigger every 50 new transcripts
